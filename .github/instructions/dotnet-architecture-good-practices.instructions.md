...
* **异步编程**: 使用 `async` 和 `await` 处理 I/O 密集型操作以确保可扩展性。严禁使用 `async void`（生命周期不可控）以及 `.Result`/`.Wait()`。
* **避免阻塞调用**: 在常规请求处理路径中避免获取重量级锁或调用同步阻塞 API。
* **依赖注入 (DI)**: 利用内置 DI 容器提升解耦性和可测试性。
* **后台任务**: 长时间运行的任务应移至后台服务（Background Services）。在后台线程中访问 Scope 服务时，必须注入 `IServiceScopeFactory` 并创建新作用域，严禁捕获 Controller 注入的 Scoped 服务（如 DbContext）。
* **HttpContext 安全**:
    - 严禁将 `HttpContext` 存储在字段中或在多线程间共享。
    - 严禁在请求完成后继续使用 `HttpContext`。
    - 后台任务中如需请求数据，应在任务启动前拷贝所需字符串/值。
* **LINQ**: 使用语言集成查询进行简洁可读的数据操作。
* **异常处理**: 实施清晰一致的策略来处理和记录错误。
* **现代 C# 特性**: 利用现代语言特性（如 records, 模式匹配）编写简洁健壮的代码。
* **对象映射**: 自定义一个静态扩展方法类来处理实体与 DTO 之间的映射，避免引入第三方库以减少依赖。
